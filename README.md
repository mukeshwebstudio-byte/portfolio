...
- Confirmed `index.html` links to CSS and JS with **relative paths** (`css/style.css`, `js/script.js`) — these work whether the site is served from the repo root (`username.github.io`) or a project subpath (`username.github.io/repo-name/`). No paths use a leading `/`, which is what typically causes 404s on project pages.
- Added a **`.nojekyll`** file to the project root. GitHub Pages runs Jekyll by default, which can ignore certain files/folders (e.g. anything starting with `_`) or otherwise interfere with a plain static build; `.nojekyll` disables that processing so every file is served as-is.
- Added an inline **favicon** (`<link rel="icon" ...>` using a data-URI SVG of the "MK" mark) so browsers stop auto-requesting `/favicon.ico` and logging a 404 for it — no new image file needed, and nothing about the design changed.
- Verified there are no broken `url(...)` references in the CSS (no local background images or self-hosted fonts are used — fonts load from Google Fonts over HTTPS) and no broken `<img>`/asset references in the HTML or JS.
- Verified file/folder names match their references exactly, including case (`css/style.css`, `js/script.js`) — GitHub Pages is served from a case-sensitive filesystem, so a mismatch like `CSS/Style.css` vs `css/style.css` would 404 even though it works locally on some machines.
